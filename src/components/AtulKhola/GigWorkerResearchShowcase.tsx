...
import { Users, Bike, Clock, Compass, FileText, Sparkles, AlertCircle, ArrowUpRight, Award, ShieldAlert, CheckCircle, Camera, Upload, Trash2 } from 'lucide-react';
...
export const GigWorkerResearchShowcase: React.FC = () => {
  ...
  // Restore photos from backend if available
  useEffect(() => {
    fetch('/api/gig-photos')
      .then(res => res.json())
      .then(data => {
        setPhotos(prev => ({
          blinkitPhoto: prev.blinkitPhoto || data.blinkit,
          autoPhoto: prev.autoPhoto || data.auto,
          deliveryPhoto: prev.deliveryPhoto || data.delivery,
        }));
      })
      .catch(() => {});
  }, []);

  const handleFileUpload = (file: File, slot: 'blinkit' | 'auto' | 'delivery') => {
    if (!file.type.startsWith('image/')) return;
    const reader = new FileReader();
    reader.onload = async (e) => {
      const dataUrl = e.target?.result as string;
      const key = slot === 'blinkit' ? 'blinkitPhoto' : slot === 'auto' ? 'autoPhoto' : 'deliveryPhoto';
      ...
      try {
        ...
        console.warn('Backend upload failed, kept in state', err);
      }
  ...
  const handleRemovePhoto = (slot: 'blinkit' | 'auto' | 'delivery', e: React.MouseEvent) => {
    e.stopPropagation();
    const key = slot === 'blinkit' ? 'blinkitPhoto' : slot === 'auto' ? 'autoPhoto' : 'deliveryPhoto';
    setPhotos(prev => {
  ...
  useEffect(() => {
    const handleUpdate = () => {
      try {
        ...
        else setPhotos({ blinkitPhoto: null, autoPhoto: null, deliveryPhoto: null });
      } catch (e) {
        console.warn(e);
      }
    };
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-mono-code text-emerald-400/90 font-bold flex items-center gap-1.5">
            ...
            <span>PRIMARY FIELD RECORDS</span>
          </span>
        </div>

        {/* Hidden inputs for each photo slot */}
        <input
          ref={blinkitInputRef}
          type="file"
          accept="image/*"
          className="hidden"
          onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'blinkit')}
        />
        <input
          ref={autoInputRef}
          type="file"
          accept="image/*"
          className="hidden"
          onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], 'auto')}
        />
        <input
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="relative bg-[#05140b] border border-white/15 rounded-2xl p-4 shadow-lg flex flex-col justify-between group">
            ...
            <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-black/80 border-2 border-dashed border-[#ffe600]/40 mb-3 flex items-center justify-center">
              {photos.blinkitPhoto ? (
                <>
                  <img 
                  ...
                  <div className="absolute top-2 right-2 flex items-center gap-1.5 bg-black/80 backdrop-blur-md p-1.5 rounded-lg border border-white/20 shadow-lg z-20">
                    ...
                    <button
                      onClick={(e) => handleRemovePhoto('blinkit', e)}
                ...
                <button
                  onClick={() => blinkitInputRef.current?.click()}
                  className="w-full h-full flex flex-col items-center justify-center p-4 text-center hover:bg-[#ffe600]/5 transition-all cursor-pointer group/btn"
                >
                  <div className="w-12 h-12 rounded-xl bg-[#ffe600]/10 border border-[#ffe600]/30 flex items-center justify-center text-[#ffe600] mb-2 group-hover/btn:scale-105 transition-transform">
                    <Bike className="w-6 h-6" />
                  </div>
                  <span className="text-xs font-mono-code font-bold text-white uppercase tracking-wide">
                    Night Rider Interview
                  </span>
                  <span className="text-[10px] text-white/60 font-mono-code mt-1 mb-3 max-w-xs leading-relaxed">
                    Blinkit Bistro partner interaction & field notepad
                  </span>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#ffe600] text-black text-xs font-bold font-mono-code shadow hover:bg-[#ffe600]/90 transition-all">
                    <Camera className="w-3.5 h-3.5" /> [ CUSTOMIZE / ADD IMAGE ]
                  </span>
                </button>
              )}
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-mono-code">
              <div>
                ...
                <span className="text-[10px] text-white/50">Earnings, fuel burn & surge</span>
              </div>
              <button
                onClick={() => blinkitInputRef.current?.click()}
                className="px-2 py-0.5 rounded text-[10px] bg-white/10 hover:bg-[#ffe600] hover:text-black text-white/80 font-mono-code transition-colors cursor-pointer"
              >
                {photos.blinkitPhoto ? 'Edit Photo' : '+ Add Image'}
              </button>
            </div>
          </div>
          ...
          <div className="relative bg-[#05140b] border border-white/15 rounded-2xl p-4 shadow-lg flex flex-col justify-between group">
            ...
            <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-black/80 border-2 border-dashed border-amber-400/40 mb-3 flex items-center justify-center">
              {photos.autoPhoto ? (
                <>
                  <img 
                  ...
                  <div className="absolute top-2 right-2 flex items-center gap-1.5 bg-black/80 backdrop-blur-md p-1.5 rounded-lg border border-white/20 shadow-lg z-20">
                    <button
                      ...
                      className="px-2 py-1 rounded bg-amber-400 text-black text-xs font-bold font-mono-code flex items-center gap-1 hover:bg-amber-300 transition-all cursor-pointer"
                      title="Change Photo"
                    >
                      <Camera className="w-3.5 h-3.5" />
                      <span>Change</span>
                    ...
                    <button
                      onClick={(e) => handleRemovePhoto('auto', e)}
                      className="p-1.5 rounded bg-red-500/20 hover:bg-red-500/40 text-red-400 transition-colors cursor-pointer"
                      title="Remove Photo"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                ...
                <button
                  onClick={() => autoInputRef.current?.click()}
                  className="w-full h-full flex flex-col items-center justify-center p-4 text-center hover:bg-amber-400/5 transition-all cursor-pointer group/btn"
                >
                  <div className="w-12 h-12 rounded-xl bg-amber-400/10 border border-amber-400/30 flex items-center justify-center text-amber-400 mb-2 group-hover/btn:scale-105 transition-transform">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-400 text-black text-xs font-bold font-mono-code shadow hover:bg-amber-300 transition-all">
                    <Camera className="w-3.5 h-3.5" /> [ CUSTOMIZE / ADD IMAGE ]
                  </span>
                </button>
              )}
            ...
            <div className="flex items-center justify-between text-xs font-mono-code">
              <div>
                ...
                <span className="text-[10px] text-white/50">Daily shifts & charging downtime</span>
              </div>
              <button
                onClick={() => autoInputRef.current?.click()}
                className="px-2 py-0.5 rounded text-[10px] bg-white/10 hover:bg-amber-400 hover:text-black text-white/80 font-mono-code transition-colors cursor-pointer"
              >
                {photos.autoPhoto ? 'Edit Photo' : '+ Add Image'}
              </button>
            </div>
          </div>
          ...
          <div className="relative bg-[#05140b] border border-white/15 rounded-2xl p-4 shadow-lg flex flex-col justify-between group">
            ...
            <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-black/80 border-2 border-dashed border-emerald-400/40 mb-3 flex items-center justify-center">
              {photos.deliveryPhoto ? (
                <>
                  ...
                  <div className="absolute top-2 right-2 flex items-center gap-1.5 bg-black/80 backdrop-blur-md p-1.5 rounded-lg border border-white/20 shadow-lg z-20">
                    ...
                    <button
                      onClick={(e) => handleRemovePhoto('delivery', e)}
                      className="p-1.5 rounded bg-red-500/20 hover:bg-red-500/40 text-red-400 transition-colors cursor-pointer"
                      title="Remove Photo"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                ...
                <button
                  onClick={() => deliveryInputRef.current?.click()}
                  className="w-full h-full flex flex-col items-center justify-center p-4 text-center hover:bg-emerald-400/5 transition-all cursor-pointer group/btn"
                >
                  <div className="w-12 h-12 rounded-xl bg-emerald-400/10 border border-emerald-400/30 flex items-center justify-center text-emerald-400 mb-2 group-hover/btn:scale-105 transition-transform">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-400 text-black text-xs font-bold font-mono-code shadow hover:bg-emerald-300 transition-all">
                    <Camera className="w-3.5 h-3.5" /> [ CUSTOMIZE / ADD IMAGE ]
                  </span>
                </button>
              )}
            ...
            <div className="flex items-center justify-between text-xs font-mono-code">
              <div>
                ...
                <span className="text-[10px] text-white/50">Order acceptance speed & ratings</span>
              </div>
              <button
                onClick={() => deliveryInputRef.current?.click()}
                className="px-2 py-0.5 rounded text-[10px] bg-white/10 hover:bg-emerald-400 hover:text-black text-white/80 font-mono-code transition-colors cursor-pointer"
              >
                {photos.deliveryPhoto ? 'Edit Photo' : '+ Add Image'}
              </button>
            </div>
          </div>
